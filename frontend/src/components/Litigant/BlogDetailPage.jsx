...
const BlogDetailPage = () => {
    ...
    useEffect(() => {
        const fetchBlogPost = async () => {
            try {
                ...
                setError(`Error: ${error.message || 'Failed to load the article.'}`);
                setLoading(false);
            }
        };
        fetchBlogPost();
    ...
    if (!blog) {
        return <p className="text-center">No blog post found.</p>;
    }
    ...
    return (
        ...
        <div className="max-w-4xl mx-auto p-4 md:p-8">
            ...
            <article className="bg-white rounded-lg shadow-md p-6">
                <h1 className="text-3xl font-bold text-orange-600 mb-4">{blog.blogTitle}</h1>
